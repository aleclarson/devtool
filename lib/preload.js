...
(function () {
  ...
  var nodeResolve = require('resolve').resolvePath;
  ...
  // pipe DevTools console back to terminal
  consoleHook();
  ...
  function doRequire (file) {
    ...
    if (globals.browserField) {
      result = browserResolve.sync(file, {
        basedir: cwd
      });
    } else {
      result = nodeResolve(file, {
        parent: cwd
      });
    }
    require(result);
  ...
  function hookProcess () {
    ...
    process.exit = function (code) {
      console.warn('The process will exit after this breakpoint!');
      debugger;
